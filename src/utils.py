...
def generate_stats(samples: Tensor, BLOCK_SIZE: int=32) -> Tensor:
    """Generate statistical measures from MCMC samples.
    
    Args:
        samples (Tensor): MCMC samples with shape [nwalkers, n_batches]
        BLOCK_SIZE (int, optional): Size of blocks for variance calculation. Defaults to 32.
    
    Returns:
        dict: Dictionary containing various statistical measures:
            - mean: Average of all samples
            - error_of_mean: Standard error of the mean
            - batch_variance: Variance between batches
            - variance: Overall variance of samples
            - R_hat: Gelman-Rubin convergence diagnostic
            - ESS: Effective Sample Size
            - tau_min/avg/std/max: Autocorrelation time statistics
    """
    if(samples.ndim==1):
...
def generate_samples(calc_elocal: Callable,
    ...
    """Generate local energy measurements using MCMC sampling.
    
    Args:
        calc_elocal (Callable): Function to calculate local energy
        sampler (nn.Module): MCMC sampler module
        n_batches (int): Number of batches to sample
        chunk_size (Optional[int]): Size of chunks for batch processing
        n_sweeps (int): Number of MCMC steps between measurements
        storage_device (Union[torch.device, str]): Device to store energy values
    
    Returns:
        Tensor: Transposed tensor of local energies with shape [nwalkers, n_batches]
               Each entry represents a local energy measurement for a given walker and batch
    """
    ...
    for batch in tqdm(range(n_batches)):
        ...
        x=x.requires_grad_(True)
        samples[batch, :] = calc_elocal(x)[0].detach_() 
    samples = samples.t()
    return samples
...
def str_with_err(value, error):
    if error == 0 or not np.isfinite(error):
        digits = 2  # fallback default
    else:
        digits = max(0, -int(np.floor(np.log10(abs(error)))))
